...
int cmd_usrgrp_sid(char * parsing_sid, char *grp, int what, int human)
{
    ...
    while (current_separator)
    {
        ...
        if ( ret > 0 )
        {
            ...
            default:
                if ( ret > 4294967295 )
                {
                    fprintf(stderr, "ERROR: SID authority more than 4294967295\n");
                    free(sid);
                    return 0;
                }
    ...
    switch (what)
    {
        case 1: ret = sam_add_sid_to_grp( hive[H_SAM], sid, numgrp); break;
...
int main(int argc, char **argv)
{
  ...
  if ( list == 0 )
  {
      ...
      if ( usr && !*usr )
      {
          fprintf(stderr,"%s: ERROR: username must be specified with -u.\n",argv[0]);
          exit(1);
      }
